...
#define MAXSTACKSIZE 11 //max stack size
...
int main(int argc, char* argv[]) {
    ...
    if (atoi(argv[4]) == 0) {
		...
		std::cout << "Core reduction time: " << duration4.count() << "s" << std::endl;
		...
		std::cout << "2-hop time: " << duration3.count() << "s" << std::endl;
		...
		std::cout << "Edge directing time: " << duration2.count() << "s" << std::endl;
		...
		std::cout << "Transform to bitmap time: " << duration1.count() << "s" << std::endl;
		...
		findCliqueGPUNewBatch << <BLOCKNUM, THREADNUM >> > (id_offset_dev_L, notzero_id_dev_L, bits_dev_L, id_offset_dev_H, notzero_id_dev_H, bits_dev_H, count_dev, p_dev, q_dev, H_size_dev, non_vertex_dev, non_vertex_dev_father);
		checkCudaErrors(cudaGetLastError());
		...
		auto start = std::chrono::high_resolution_clock::now();
		findCliqueGPUNewBatch << <BLOCKNUM, THREADNUM >> > (id_offset_dev_L, notzero_id_dev_L, bits_dev_L, id_offset_dev_H, notzero_id_dev_H, bits_dev_H, count_dev, q_dev, p_dev, H_size_dev, non_vertex_dev, non_vertex_dev_father);
		checkCudaErrors(cudaGetLastError());
		cudaMemcpy(&count, count_dev, sizeof(unsigned long long), cudaMemcpyDeviceToHost);
		checkCudaErrors(cudaGetLastError());
		auto end = std::chrono::high_resolution_clock::now();
